...
export async function POST() {
  ...
  // Traemos ventas del día con toda la cadena necesaria:
  // sale.items -> (saleItem) -> recipe -> recipe.items -> include { item: true }
      ...
      items: {
        include: {
          recipe: {
            include: {
              items: {
                include: { item: true } // ✅ ahora ri.item existe en tipos y en datos
              }
            }
          }
        }
  ...
  for (const s of sales) {
    ...
    for (const it of s.items) {
      ...
      const unitCost = (it.recipe?.items ?? []).reduce((acc, ri) => {
        const costPerUnit = ri.item?.costPerUnit ?? 0
        const qty = ri.quantity ?? 0
        return acc + (qty * costPerUnit)
      }, 0)
  ...
  const closing = await prisma.cashClosing.create({
    data: {
      ...
      tickets,
      itemsSold,
      byMethod: JSON.stringify(byMethod) // si el campo es String en Prisma
    }
  })
